...
while true; do
    ...
    case ${output:0:1} in
        s|S)
            ...
            output_file_01="$path/LinuxAudit_ConnectedUsers_$fecha.txt"
			output_file_02="$path/LinuxAudit_RuningServices_$fecha.txt"
			output_file_03="$path/LinuxAudit_ActiveConnections_$fecha.txt"
			output_file_04="$path/LinuxAudit_DiskFreeSpace_$fecha.txt"
			output_file_05="$path/LinuxAudit_History_$fecha.txt"
            output_file_06="$path/LinuxAudit_ActiveProcess_$fecha.txt"
			output_file_07="$path/LinuxAudit_PasswordPolicies_$fecha.txt"
			output_file_08="$path/LinuxAudit_UserList_$fecha.txt"
			output_file_09="$path/LinuxAudit_NULL_Passwords_$fecha.txt"
			output_file_10="$path/LinuxAudit_FailSessionStart_$fecha.txt"
			output_file_11="$path/LinuxAudit_Cat_to_Shadow_$fecha.txt"
            break
            ;;
        n|N)
...
volcar_historial() {
    local usuario="$1"
    local archivo_salida="$path/LinuxAudit_history_${usuario}_${fecha}.txt"

    if [ -f "/home/$usuario/.bash_history" ]; then
        cp "/home/$usuario/.bash_history" "$archivo_salida"
